...
const TutoringSessions = () => {
  ...
  const fetchSessions = async (pageNumber, query, status) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/administration/content/tutoring-sessions/?${params.toString()}`);
      const data = await response.json();

      if (!response.ok || !data.success) {
        throw new Error(data.message || 'Failed to load tutoring sessions.');
      }

      setSessions(data.sessions || []);
      setSummary(data.summary || null);
      setPagination(data.pagination || { current_page: pageNumber, total_pages: 1, total_items: data.sessions?.length || 0 });
    } catch (err) {
      setError(err.message || 'Failed to load tutoring sessions.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AdminLayout pageTitle="Tutoring Sessions" pageDescription="Monitor tutoring session pipeline and status.">
      <div className="space-y-6">
        <div className="rounded-xl border border-[#E7F3FB] bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            {summary && (
              <div className="grid w-full gap-3 rounded-lg border border-[#E7F3FB] bg-[#F5F9FD] px-4 py-3 text-sm text-[#123460] md:w-auto md:grid-cols-5">
                <div>
                ...
                <div>
                  ...
                  <p className="font-semibold">{numberFormatter.format(summary.pending_sessions || 0)}</p>
                </div>
                <div>
                ...
                <div>
                  <p className="text-xs uppercase text-[#7F8B99]">Cancelled</p>
                  <p className="font-semibold">{numberFormatter.format(summary.cancelled_sessions || 0)}</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="rounded-xl border border-[#E7F3FB] bg-white shadow-sm">
          {loading ? (
            <div className="flex h-60 items-center justify-center gap-3 text-[#7F8B99]">
              ...
              <span>Loading tutoring sessions...</span>
            </div>
          ) : error ? (
                    ...
                    {sessions.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="px-4 py-6 text-center text-sm text-[#7F8B99]">
                          No tutoring sessions found for the current filters.
                        </td>
                      </tr>
                    ) : (
                      sessions.map((session) => (
                        <tr key={session.id} className="hover:bg-[#F5F9FD]">
                          <td className="px-4 py-3 font-medium">{session.tutor || '—'}</td>
                          <td className="px-4 py-3">{session.subject || '—'}</td>
                          <td className="px-4 py-3 capitalize">{session.status || '—'}</td>
                          <td className="px-4 py-3">{formatDate(session.scheduled_at)}</td>
                          <td className="px-4 py-3">{session.duration_minutes || '—'}</td>
