          ...
          subjects: [
            {
              ...
              syllabus: {
                overview:
                  "Human body structure, organ systems, histology basics. This comprehensive course covers the fundamental structures of the human body.",
                objectives: [
          ...
          subjects: [
            {
              ...
              syllabus: {
                overview:
                  "Fundamental data structures and their applications in computer science.",
                objectives: [
...
const AcademicContent = () => {
  ...
  const UNIVERSITY_ID = 1; // current university scope
  ...
  // Load initial faculties, their programs, and summary from backend
  useEffect(() => {
    async function load() {
      try {
        const [facRes, sumRes] = await Promise.all([
          getFaculties(UNIVERSITY_ID),
          getSummary(UNIVERSITY_ID),
        ]);
        const facultiesRaw = facRes.faculties || [];
        const programsByFaculty = await Promise.all(
          facultiesRaw.map((f) =>
            getProgramsByFaculty(f.id).catch(() => ({ programs: [] }))
          )
        );
        const faculties = facultiesRaw.map((f, idx) => ({
          ...f,
          courses: (programsByFaculty[idx].programs || []).map((p) => ({
            id: p.id,
            name: p.title,
            duration: "-",
            intake: "N/A",
        ...
        if (sumRes && sumRes.success) {
          setSummary({
            faculty_count: sumRes.faculty_count || 0,
            course_count: sumRes.course_count || 0,
            subject_count: sumRes.subject_count || 0,
          });
        }
      } catch (e) {
        console.error("Failed to load academic content", e);
      }
    }
    load();
  }, []);

  // When selecting a faculty, fetch its programs (courses)
  const handleSelectFaculty = async (faculty) => {
    setSelectedFaculty(faculty);
    setSelectedCourse(null);
    setSelectedSubject(null);
    try {
      const res = await getProgramsByFaculty(faculty.id);
      const programs = (res.programs || []).map((p) => ({
        id: p.id,
        name: p.title,
        duration: "-", // optional: wire DegreeProgramDurations later
        intake: "N/A",
        subjects: [],
      }));
      setData((prev) => {
        const faculties = (prev.faculties || []).map((f) =>
          f.id === faculty.id ? { ...f, courses: programs } : f
        );
  ...
  // When selecting a course (program), fetch its subjects
  const handleSelectCourseRemote = async (course) => {
    setSelectedCourse(course);
    setSelectedSubject(null);
    try {
      const res = await getSubjectsByProgram(course.id);
      const subjects = (res.subjects || []).map((s) => ({
        id: s.id,
        name: s.name,
        code: s.code,
        credits: s.credits ?? 0,
        syllabus: {
          overview: s.overview?.overview || s.description || "",
          objectives: s.overview?.objectives || [],
          topics: s.overview?.topics || [],
          assessment: s.overview?.assessment || [],
          references: s.overview?.references || [],
        },
        content: (s.files || []).map((f) => f.name),
        _files: s.files || [],
      }));
      setData((prev) => {
        const faculties = (prev.faculties || []).map((f) => {
          if (!selectedFaculty || f.id !== selectedFaculty.id) return f;
          return {
            ...f,
            courses: (f.courses || []).map((c) =>
              c.id === course.id ? { ...c, subjects } : c
            ),
          };
        });
        return { ...prev, faculties };
  ...
  const handleDeleteCourse = (courseId) => {
    if (window.confirm("Are you sure you want to delete this course?")) {
      ...
      const faculty = updatedData.faculties.find(
  ...
  const handleAddCourse = async () => {
    ...
    try {
      ...
      const progRes = await getProgramsByFaculty(selectedFaculty.id);
      const programs = (progRes.programs || []).map((p) => ({
        id: p.id,
        name: p.title,
        duration: "-",
        intake: "N/A",
      ...
      setData((prev) => ({
        faculties: (prev.faculties || []).map((f) =>
          f.id === selectedFaculty.id ? { ...f, courses: programs } : f
        ),
      }));
      const sumRes = await getSummary(UNIVERSITY_ID);
      ...
      >
        ...
        >
          <div style={{ marginBottom: "2rem" }}>
            ...
            >
              Manage faculties, courses, subjects, and academic content across
              the university
            </p>
          </div>
          <div
          ...
          >
            ...
            >
              ...
              >
                {summary.faculty_count ?? data.faculties.length}
              </h3>
              <p
                style={{
                  color: "#64748b",
          ...
          >
            {data.faculties.map((faculty) => (
              <div
                ...
                style={{
                  background:
                    selectedFaculty?.id === faculty.id ? "#eef2ff" : "#f9fafb",
                  border: `2px solid ${
                    selectedFaculty?.id === faculty.id ? "#6366f1" : "#e5e7eb"
                  }`,
                  borderRadius: "8px",
                  padding: "1.5rem",
                  cursor: "pointer",
              ...
              {selectedFaculty.courses.map((course) => (
                <div
                  ...
                  style={{
                    ...
                    alignItems: "center",
                    background:
                      selectedCourse?.id === course.id ? "#eef2ff" : "#f9fafb",
                    border: `2px solid ${
                      selectedCourse?.id === course.id ? "#6366f1" : "#e5e7eb"
                    }`,
                    borderRadius: "8px",
                    padding: "1.5rem",
                    cursor: "pointer",
                  ...
                  onClick={() => handleSelectCourseRemote(course)}
                >
                  <div>
                    <h4
                      style={{
                  ...
                  style={{
                    ...
                    background:
                      selectedSubject?.id === subject.id
                        ...
                        : "#f9fafb",
                    border: `2px solid ${
                      selectedSubject?.id === subject.id ? "#6366f1" : "#e5e7eb"
                    }`,
                    borderRadius: "8px",
                    padding: "1.5rem",
                    cursor: "pointer",
        ...
        {selectedSubject && (
          ...
          >
            ...
            >
              ...
              >
                Academic Content for {selectedSubject.name}
              </h2>
              <button
          ...
          >
            ...
            >
              <div>
                ...
                >
                  📝 Edit Syllabus - {selectedSubject.name}
                </h3>
                <div
                  style={{
                    fontSize: "0.875rem",
                ...
                >
                  Course Code: {selectedSubject.code} | Credits:{" "}
