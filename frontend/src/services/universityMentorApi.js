...
export async function fetchMentorRequests(universityId) {
  const res = await fetch(`${API}/mentor-requests/?university_id=${encodeURIComponent(universityId)}`);
  return res.json();
}

export async function fetchActiveMentors(universityId) {
  const res = await fetch(`${API}/active-mentors/?university_id=${encodeURIComponent(universityId)}`);
  return res.json();
}

export async function acceptMentor(preMentorId) {
  const res = await fetch(`${API}/mentor-requests/${preMentorId}/accept/`, { method: 'POST' });
  return res.json();
}
