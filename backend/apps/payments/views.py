...
from apps.universities.models import Universities, UniversityAd
from .models import CompanyAdPayments, UniversityAdPayments
...
@csrf_exempt
@transaction.atomic
def university_ad_payment(request):
    """Process a university ad payment and persist both the ad and the payment.
    Request body: { ad: {university_id, ...}, payment: {payment_method, email, amount?} }
    """
    if request.method != 'POST':
        return JsonResponse({'success': False, 'message': 'Only POST method allowed'}, status=405)
    ...
    try:
        ...
        university_id = ad_data.get('university_id')
        ...
        amount = Decimal(str(pay_data.get('amount', ad.budget or 0)))
        payment = UniversityAdPayments.objects.create(
            ad=ad,
            amount=amount,
            payment_method=pay_data.get('payment_method', 'credit-card'),
            transaction_id=pay_data.get('transaction_id'),
