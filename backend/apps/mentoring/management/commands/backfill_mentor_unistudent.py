...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        updated = 0
        skipped = 0
        qs = Mentors.objects.select_related(
            'user').filter(university_student__isnull=True)
        for m in qs:
            if not m.user_id:
                skipped += 1
                continue
            stu = UniversityStudents.objects.filter(
                user_id=m.user_id).order_by('university_student_id').first()
            if not stu:
                skipped += 1
                continue
        ...
        self.stdout.write(self.style.SUCCESS(
            f"Backfilled {updated} mentors; skipped {skipped}"))
