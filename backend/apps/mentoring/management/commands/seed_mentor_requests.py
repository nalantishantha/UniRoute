...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--university-id', type=int, required=True, help='University ID to seed for')
        parser.add_argument('--count', type=int, default=3, help='How many mentor requests to create')
        parser.add_argument('--approve', type=int, default=0, help='Number of mentors to auto-approve (become active)')
    ...
    def handle(self, *args, **options):
        ...
        students = list(UniversityStudents.objects.select_related('user').filter(university_id=uni_id))
        if not students:
            raise CommandError(f"No UniversityStudents found for university_id={uni_id}")
        ...
        for i in range(count):
            ...
            skills = random.sample(['Python', 'React', 'Django', 'Node.js', 'AWS', 'Docker', 'SQL', 'ML'], k=3)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--university-id', type=int, required=True, help='Target university_id')
        parser.add_argument('--count', type=int, default=3, help='How many requests to create')
        parser.add_argument('--approve', type=int, default=1, help='How many to auto-approve into active mentors')
    ...
    def handle(self, *args, **options):
        ...
        students = list(UniversityStudents.objects.select_related('user', 'university', 'degree_program').filter(university_id=uid)[:count])
        if not students:
            raise CommandError('No UniversityStudents found for university_id=%s. Create some first.' % uid)
