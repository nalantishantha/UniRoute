...
@csrf_exempt
def create_university_ad(request):
    """Create a new university advertisement (mirrors create_company_ad)."""
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            ...
            ad = UniversityAd.objects.create(
                university=uni,
...
@csrf_exempt
def get_university_ads(request):
    """Get all ads for a university"""
    if request.method == 'GET':
        university_id = request.GET.get('university_id')
        ads = UniversityAd.objects.filter(
            university_id=university_id).order_by('-created_at')
        data = []
        for ad in ads:
            data.append({
                'ad_id': ad.ad_id,
...
@csrf_exempt
def update_university_ad(request, ad_id):
    """Update an existing university ad"""
    if request.method == 'PUT':
        try:
            data = json.loads(request.body)
            ad = UniversityAd.objects.get(ad_id=ad_id)
            for field, value in data.items():
                setattr(ad, field, value)
            ad.save()
            return JsonResponse({'success': True})
        except UniversityAd.DoesNotExist:
...
@csrf_exempt
def delete_university_ad(request, ad_id):
    """Delete a university ad"""
    if request.method == 'DELETE':
        try:
            ad = UniversityAd.objects.get(ad_id=ad_id)
            ad.delete()
...
@csrf_exempt
def upload_university_ad_media(request):
    """Accept image/video via multipart/form-data and return a local URL (dev helper)."""
    if request.method != 'POST':
        return JsonResponse({'success': False, 'message': 'Only POST method allowed'}, status=405)
